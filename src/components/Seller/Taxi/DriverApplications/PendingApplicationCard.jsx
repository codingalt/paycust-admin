...
import { useApproveDriverMutation, useRejectDriverMutation } from '@/services/api/seller/taxi/taxiApi';
import Loader from '@/components/Loader/Loader';
import { toastError, toastSuccess } from '@/components/Toast/Toast';
import ImageComponent from '@/components/ui/ImageComponent';

const PendingApplicationCard = ({data}) => {
  const [approveDriver, res] = useApproveDriverMutation();
  const {isLoading, error, isSuccess} = res;

  useMemo(()=>{
    if(isSuccess){
      toastSuccess("Driver Application was Approved")
    }
  },[isSuccess]);
  ...
  // Reject Driver 
  const [rejectDriver, resp] = useRejectDriverMutation();
  const { isLoading: isLoadingReject, error: rejectError, isSuccess: isRejectSuccess } = resp;

  useMemo(() => {
    if (isRejectSuccess) {
      toastSuccess("Driver Application was Rejected");
    }
  }, [isRejectSuccess]);
  ...
  const handleApprove = async(driver_id)=>{
    await approveDriver(driver_id);
  }

  const handleReject = async (driver_id) => {
    await rejectDriver(driver_id);
  };

  return (
    <Card className="flex-1 max-w-2xl shadow-sm border-solid border-1 p-4">
      {/* Driver Details  */}
